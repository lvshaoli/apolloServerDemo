...
async function set ({ id, projectId, value, disk = false }, context) {
    ...
    stat.value = 0
    context.pubsub.publish(channels.SHARED_DATA_UPDATED, { sharedDataUpdated: { id, projectId, value } })
...
async function remove ({ id, projectId }, context) {
    ...
    context.pubsub.publish(channels.SHARED_DATA_UPDATED, {sharedDataUpdated: { id, projectId, value: undefined }})
